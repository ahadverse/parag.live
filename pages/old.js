...
const Name = () => {
  ...
  if (router.query.category) {
    if (router.query?.category?.[0] == undefined) {
      ...
      content = (
        <>
          <div
            className='flex flex-wrap items-center px-0 sm:px-32'
  ...
  return (
    <div className='page-bg' style={{ color: "var(--text)" }}>
      <Head>
        ...
        <meta
          ...
          content={
            router?.query?.category &&
            `${router?.query?.category} Backpage | After backpage, PARAG is the most popular classified site for ${router?.query?.category} Escorts. 
PARAG is a free adult entertainment site for ${router?.query?.category} Escorts or Hardcore in ${router?.query?.category}.
Hookup hotshot and erotic massage services in ${router?.query?.category} on PARAG.
Explore PARAG.com now!`
          }
        />
        <meta
          name='keywords'
      ...
      {links ? (
        <div
          className='flex justify-around text-xl p-2'
